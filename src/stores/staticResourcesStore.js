...
import useGridStore from './staticGridStore';
...
const useStaticResourcesStore = create(
  persist(
    (set, get) => ({
      ...
      // get notifications
      updateNotifications: () => {
        const { tiles } = useGridStore.getState();
      ...
      checkWinConditions: () => {
        ...
        const { population, wildlife, trees, pollution, funds } = resources;
        const { setCurrentModal, setEndModalReason } = useGridStore.getState();
        ...
        const groundwaterPollutionAcceptable =
          pollution.groundwater.current <= pollution.groundwater.threshold;
        ...
        const allConditionsMet = [
          ...
          groundwaterPollutionAcceptable,
          satisfactionMet,
        ...
        if (insufficientFunds || populationReachedButOthersFailed) {
          ...
          if (insufficientFunds) {
            ...
            else if (!groundwaterPollutionAcceptable)
              loseReason = 'groundwaterPollutionTooHigh';
      ...
      updateResourcesForGreenMods: (tileKey, newGreenMods) => {
        ...
        const { updateTileMetrics } = useGridStore.getState();
      ...
      updateCurrentTemperature: () => {
        const { tiles, gridSize } = useGridStore.getState();
        const { resources } = useStaticResourcesStore.getState();
...
export default useStaticResourcesStore;
