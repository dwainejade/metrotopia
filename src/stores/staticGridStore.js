...
const placeRandomBuildings = () => {
  const store = useStaticGridStore.getState();
      ...
      if (gameObjects.naturalFeatures[currentTile.type]) {
        ...
        if (Math.random() < replacementChance) {
          ...
          if (success && Math.random() < GREEN_MOD_PROBABILITY) {
            ...
            // Apply the mods
            useStaticGridStore.getState().applyMods(key, {
              ...mods,
  ...
  const urbanRatio = useStaticGridStore.getState().calculateUrbanRatio();
...
const useStaticGridStore = create(
  persist(
    (set, get) => ({
      ...initialState,
...
export const useStaticGridStoreInitialization = () => {
  React.useEffect(() => {
    const store = useStaticGridStore.getState();
...
export default useStaticGridStore;
