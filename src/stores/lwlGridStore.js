...
import generateMetricsForTile from "../helpers/generateLwlMetricsForTile";
import gameObjects, {
	assignRandomModsToCities,
	greenModMetrics,
	powerModMetrics,
	transportationConnectionModifiers,
} from "../data/lwlGameObjects";
import {
...
const placeRandomBuildings = () => {
	const store = useLwlGridStore.getState();
	const audioStore = useAudioStore.getState();
	const resourceStore = useLwlResourcesStore.getState();
			...
			if (gameObjects.naturalFeatures[currentTile.type]) {
				...
				if (Math.random() < replacementChance) {
					...
					if (success && Math.random() < GREEN_MOD_PROBABILITY) {
						...
						// Apply the mods
						useLwlGridStore.getState().applyMods(key, {
							...mods,
	...
	const urbanRatio = useLwlGridStore.getState().calculateUrbanRatio();
	...
	persist(
		(set, get) => ({
			...
			resetGame: () => {
				...
				useLwlResourcesStore.getState().resetResources();
				generateRandomGrid();
			...
			applyConnectionMod: (tileKey, modType, value) => {
				set((state) => {
					...
					useLwlResourcesStore.getState().updateResources(tile, updatedTile);
			...
			placeObject: (selectedCell, newTileType, powerMod) => {
				...
				if (selectedCell && oldCellData.type !== newTileType) {
					...
					// Metrics for lizards
					newMetrics.wildlife.reptilesAndAmphibians =
						prevMetrics.wildlife.reptilesAndAmphibians -
						Math.floor(
							newMetrics.wildlife.reptilesAndAmphibians *
								prevMetrics.wildlife.reptilesAndAmphibians
						);
					...
					// Update pollution
					useLwlResourcesStore.getState().updateTotalPollution();
					useLwlResourcesStore.getState().updateCurrentTemperature();
					useLwlResourcesStore.getState().calculateSurfaceWater();
					useLwlResourcesStore.getState().recalculateConnectionMods();
			...
			canBuild: (tileKey, newBuildingType) => {
				...
				const resourcesStore = useLwlResourcesStore.getState();
			...
			generateRandomGrid: () => {
				...
				useLwlResourcesStore.getState().initializeResources(processedTiles);
				useLwlResourcesStore.getState().updateCurrentTemperature();
				useLwlResourcesStore.getState().calculateSurfaceWater();
			},
			...
			updatePowerMod: (tileKey, newPowerMod) => {
				...
				useLwlResourcesStore.getState().updateEnergyUsage(tile, newPowerMod);
				useLwlResourcesStore.getState().updateTotalPollution();
				useLwlResourcesStore.getState().updateCurrentTemperature();
			},
...
export const useLwlGridStoreInitialization = () => {
	React.useEffect(() => {
		const store = useLwlGridStore.getState();
...
export default useLwlGridStore;
