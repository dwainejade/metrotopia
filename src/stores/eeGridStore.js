...
import useResourcesStore from './eeResourcesStore';
import useCameraStore from './cameraStore';
import generateMetricsForTile from '@src/helpers/generateMetricsForTile';
import gameObjects, {
...
import {
  ...
  collectBuildingsForNewTile,
  propagateBuildingsToNeighbors,
  findConnectedBuildingTypes,
} from '@src/helpers/propagateModsEE';
import { spaceEachWord } from '@src/helpers/capitalizeEachWord';
    ...
    (set, get) => ({
      ...
      placeObject: (selectedCell, newTileType, powerMod) => {
        ...
        if (selectedCell && oldCellData.type !== newTileType) {
          ...
          // Add building groups
          const connectedBuildingGroups = [null];
          ...
          const collectedBuildings = collectBuildingsForNewTile(
            tiles,
            selectedCell,
            gridSize,
            newTileType
          ...
          const allBuildings = propagateBuildingsToNeighbors(
            newTiles,
            gridSize,
            selectedCell,
            collectedBuildings
          ...
          const updatedRetailConnections = findConnectedBuildingTypes(newTiles);

          // Update the store
          set({
            tiles: newTiles,
            selectedTile: null,
            retailConnections: updatedRetailConnections,
          });
          ...
          useResourcesStore
            .getState()
            .updateConnections(updatedRetailConnections);
