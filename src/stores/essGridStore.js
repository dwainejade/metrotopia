...
import useResourcesStore from "./essResourcesStore";
...
const placeRandomBuildings = () => {
	const store = useEssGridStore.getState();
			...
			if (gameObjects.naturalFeatures[currentTile.type]) {
				...
				if (Math.random() < replacementChance) {
					...
					if (success && Math.random() < GREEN_MOD_PROBABILITY) {
						...
						// Apply the mods
						useEssGridStore.getState().applyMods(key, {
							...mods,
	...
	const urbanRatio = useEssGridStore.getState().calculateUrbanRatio();
		...
		(set, get) => ({
			...
			placeObject: (selectedCell, newTileType, powerMod) => {
				...
				if (selectedCell && oldCellData.type !== newTileType) {
					...
					// Directly update the tile's temperature metrics instead of using global temperature
			...
			initializeStore: () => {
				const state = get();
				const storedData = localStorage.getItem("city-grid-ess");
...
export default useEssGridStore;
