...
import gameObjects, { greenModMetrics } from "@src/data/eeGameObjects";
import useStore from "./eeGridStore";
import randomizeInRange from "@src/helpers/randomizeNumbers";
import {
	calculateSatisfaction,
	generateBuildingSuggestion,
} from "@src/helpers/calculateSatisfaction";
...
const initialResourceState = {
	...
	wildlife: {
		...
		totalLost: 0,
		percentPreserved: 100,
		targetPreservation: 80, // target percent
...
const useMainResourcesStore = create(
	persist(
		(set, get) => ({
			...
			// Update the retail connections
			updateConnections: (newConnections) => {
				let percentages = {
					buildingConnectionPercent: 0,
					transportationConnectionPercent: 0,
				};

				newConnections.forEach((center) => {
					let tempPercents = {
						buildingConnectionPercent: 0,
						transportationConnectionPercent: 0,
					};

					tempPercents.transportationConnectionPercent =
						center.transportation.length * 25;

					if (
						center.buildingTypes.includes("residential") &&
						center.buildingTypes.includes("commercial")
					) {
						tempPercents.buildingConnectionPercent += 25;
					}
					if (
						center.buildingTypes.includes("industrial") &&
						center.buildingTypes.includes("commercial")
					) {
						tempPercents.buildingConnectionPercent += 25;
					}

					// Set percentage to max numbers
					percentages.buildingConnectionPercent = Math.max(
						percentages.buildingConnectionPercent,
						tempPercents.buildingConnectionPercent
					);
					percentages.transportationConnectionPercent = Math.max(
						percentages.transportationConnectionPercent,
						tempPercents.transportationConnectionPercent
					);
				});
			...
			checkWinConditions: () => {
				...
				const { population, wildlife, trees, pollution, funds, taxes } =
					resources;
				const { setCurrentModal, setEndModalReason } = useStore.getState();

				updateSatisfaction();

				// Win conditions
				const populationReached = population.total >= population.target;
				const treesPreserved = trees.percentPreserved >= trees.targetPreserved;
				const wildlifePreserved =
					wildlife.percentPreserved >= wildlife.targetPreservation;
				const airPollutionAcceptable =
					pollution.air.current <= pollution.air.threshold;
				const satisfactionMet = satisfaction.score >= satisfaction.threshold;
				const connectionsReached = connectionPercentage >= connectionTarget;
				const taxRevenueMet =
					taxes.income + taxes.sales + taxes.property >= taxes.target;

				const allConditionsMet = [
					populationReached,
					treesPreserved,
					wildlifePreserved,
					airPollutionAcceptable,
					satisfactionMet,
					connectionsReached,
					taxRevenueMet,
				...
				const insufficientFunds = funds.current < 5000;
				const populationReachedButOthersFailed =
					populationReached && !allConditionsMet;

				if (insufficientFunds || populationReachedButOthersFailed) {
					if (gameLost) return;
					set({ gameLost: true });
					setCurrentModal("End");
					...
					if (insufficientFunds) {
						loseReason = "insufficientFunds";
					} else if (populationReachedButOthersFailed) {
						if (!treesPreserved) loseReason = "treesNotPreserved";
						else if (!wildlifePreserved) loseReason = "wildlifeNotPreserved";
						else if (!airPollutionAcceptable)
							loseReason = "airPollutionTooHigh";
						else if (!satisfactionMet) loseReason = "lowSatisfaction";
						else if (!connectionsReached)
							loseReason = "insufficientConnections";
						else if (!taxRevenueMet) loseReason = "insufficientTaxRevenue";
					}
			...
			updateResourcesForGreenMods: (tileKey, newGreenMods) => {
				...
				const { updateTileMetrics } = useStore.getState();
			...
			updateCurrentTemperature: () => {
				const { tiles, gridSize } = useStore.getState();
				const { resources } = useMainResourcesStore.getState();
			...
			// recalculateTreeCount: () => {
			// 	const { tiles } = useStore.getState();
			// 	const { resources } = get();
			// 	let totalTrees = 0;

			// 	Object.values(tiles).forEach((tile) => {
			// 		totalTrees += tile.metrics.treeCount || 0;
			// 	});

			// 	const lostTrees = resources.trees.startingTrees - totalTrees;

			// 	// Correct the logic: if startingTrees is 0, set percentPreserved to 100
			// 	const percentPreserved =
			// 		resources.trees.startingTrees > 0
			// 			? Math.min(
			// 				100,
			// 				Math.max(0, (totalTrees / resources.trees.startingTrees) * 100)
			// 			)
			// 			: 100;

			// 	resources.trees.currentTrees = totalTrees;
			// 	resources.trees.treesLost = Math.max(0, lostTrees);
			// 	resources.trees.percentPreserved = percentPreserved;
		...
		{
			name: "city-resources-ee",
			storage: createJSONStorage(() => localStorage),
		}
	)
);

export default useMainResourcesStore;
