...
import useResourcesStore from "./staticResourcesStore";
...
import { templateLayout } from "../data/templateLayout";
...
const useAagGridStore = create(
	persist(
		(set, get) => ({
			...
			startGame: () => {
				const { resetGame, generateLayoutFromTemplate } = get();
				resetGame();
				generateLayoutFromTemplate(templateLayout);
				set({ screen: "game" });
			},
			...
			// generae layout for static template
			generateLayoutFromTemplate: (template) => {
				const store = useAagGridStore.getState();
				const audioStore = useAudioStore.getState();

				// First, generate the random nature grid as starting point
				store.generateRandomGrid();

				// Get updated state after grid generation
				const { gridSize, placeObject, tiles, neighboringCities } = useAagGridStore.getState();
				...
				template.forEach((row, rowIndex) => {
					row.forEach((tileType, colIndex) => {
						...
						// Only replace if we have a valid building type
						if (tileType && tileType !== 'emptyTile') {
							// Skip natural features if they match what we want
							const currentTile = tiles[key];
							if (currentTile && currentTile.type === tileType) {
								return;
							}
							...
							if (collectedMods.power && collectedMods.power.size > 0) {
								...
								powerMod = powerMods[0]; // Take first available power mod
							}

							// Place the building
							placeObject(key, tileType, powerMod);
						}
					});
				});
			...
			initializeStore: () => {
				const state = get();
				const storedData = localStorage.getItem("city-grid-at-a-glance");
...
export const useAagGridStoreInitialization = () => {
	React.useEffect(() => {
		const store = useAagGridStore.getState();
...
export default useAagGridStore;
